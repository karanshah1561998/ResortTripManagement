CREATE PROCEDURE DisplayPendingPayments ()
BEGIN
    -- Directly compute pending payments using a single query
    SELECT 
    ...
    HAVING 
        `Pending Payment` > 0; -- Include only rows with pending payments
END